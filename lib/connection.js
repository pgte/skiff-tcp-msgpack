...
C._onConnect = function _onConnect(c) {
  ...
  c.on('data', function(d) {
    console.log('gotz some data from the pipez', c);
  });

  this.decoder = msgpack.decoder();
  this.encoder = msgpack.encoder();

  this.encoder.on('data', function(d) {
    console.log('encoder out:', d);
  });

  this.encoder.pipe(c).pipe(this.decoder);

  this.encoder.write({hello: self.localNodeId});
  ...
  function onMessage(m) {
    console.log('some message from decoder:', m);
    self.inQueue.push(m);
  }
};
...
C._send = function _send(type, args, cb) {
  ...
  this._onceConnected(function(c) {
    console.log('send: got connection');
...
C._receive = function _receive(fn) {
  ...
  function onReply() {
    ...
    self._onceConnected(function(c) {
      self.encoder.write({response: args});
    });
  }
